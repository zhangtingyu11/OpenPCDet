...
added_points = np.fromfile('added_lidar.bin', dtype=np.float32).reshape(-1, 3)
points_3dbox = np.fromfile('box_corners.bin', dtype=np.float32).reshape(8, 3)
# center_x = np.mean(points_3dbox[:, 0], axis=0)
# center_y = np.mean(points_3dbox[:, 1], axis=0)
# center_z = np.mean(points_3dbox[:, 2], axis=0)
# points_3dbox[:, 0]-=center_x
# points_3dbox[:, 1]-=center_y
# points_3dbox[:, 2]-=center_z
# lidar_points = lidar_points[:, :3]
lidar_points = np.concatenate([added_points, lidar_points[:, :3]], axis=0)
