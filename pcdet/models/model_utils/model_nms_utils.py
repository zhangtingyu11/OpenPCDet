...
def class_agnostic_nms(box_scores, box_preds, nms_config, score_thresh=None):
    ...
    Args:
        box_scores (_type_): 包围框分数, 
        box_preds (_type_): 预测的包围框
        nms_config (_type_): nms配置
            NMS_TYPE: NMS类型
            MULTI_CLASSES_NMS: 是否对不同类别进行NMS, 还是放在一起做NMS
            NMS_PRE_MAXSIZE: 先挑选置信度最高的多少个预测框
            NMS_POST_MAXSIZE: NMS后保留的包围框
            NMS_THRESH: NMS里的iou阈值
        score_thresh (_type_, optional): 置信度阈值. Defaults to None.

    Returns:
        selected: 挑选的roi的索引, [roi个数]
        box_scores: 挑选的roi的分数
    """
    ...
    if box_scores.shape[0] > 0:
        #* 找到置信度最高的NMS_PRE_MAXSIZE个预测框
        box_scores_nms, indices = torch.topk(box_scores, k=min(nms_config.NMS_PRE_MAXSIZE, box_scores.shape[0]))
        boxes_for_nms = box_preds[indices]
        keep_idx, selected_scores = getattr(iou3d_nms_utils, nms_config.NMS_TYPE)(
                boxes_for_nms[:, 0:7], box_scores_nms, nms_config.NMS_THRESH, **nms_config
        )
        #* 挑选NMS后置信度最高的NMS_POST_MAXSIZE个预测框
