...
from pcdet.utils.loss_utils import WeightedFocalLoss
try:
...
class WeightedDDNLoss(nn.Module):
    ...
    def __init__(self,
        ...
        self.alpha = alpha
        self.gamma = gamma
        self.loss_func = WeightedFocalLoss(alpha=self.alpha, gamma=self.gamma, reduction="none")
        self.weight = weight

    def forward(self, depth_logits, depth_maps, depth_weight):
        """
        Gets DDN loss
        Args:
            depth_logits: (B, D+1, H, W), Predicted depth logits
        ...
        # Compute loss
        loss = self.loss_func(depth_logits, depth_target, depth_weight)
