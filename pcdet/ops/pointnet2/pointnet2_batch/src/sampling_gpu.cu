...
void furthest_point_sampling_weights_kernel_launcher(int b, int n, int m,
    const float *dataset, const float *weights, float *temp, int *idxs) {
    // xyz: (B, N, 3)
    // weights: (B, N)
    // tmp: (B, N)
    // output:
    ...
    switch (n_threads) {
        case 1024:
        furthest_point_sampling_weights_kernel<1024><<<b, n_threads>>>(b, n, m, dataset, weights, temp, idxs); break;
        case 512:
        furthest_point_sampling_weights_kernel<512><<<b, n_threads>>>(b, n, m, dataset, weights, temp, idxs); break;
        case 256:
        furthest_point_sampling_weights_kernel<256><<<b, n_threads>>>(b, n, m, dataset, weights, temp, idxs); break;
        case 128:
        furthest_point_sampling_weights_kernel<128><<<b, n_threads>>>(b, n, m, dataset, weights, temp, idxs); break;
        case 64:
        furthest_point_sampling_weights_kernel<64><<<b, n_threads>>>(b, n, m, dataset, weights, temp, idxs); break;
        case 32:
        furthest_point_sampling_weights_kernel<32><<<b, n_threads>>>(b, n, m, dataset, weights, temp, idxs); break;
        case 16:
        furthest_point_sampling_weights_kernel<16><<<b, n_threads>>>(b, n, m, dataset, weights, temp, idxs); break;
        case 8:
        furthest_point_sampling_weights_kernel<8><<<b, n_threads>>>(b, n, m, dataset, weights, temp, idxs); break;
        case 4:
        furthest_point_sampling_weights_kernel<4><<<b, n_threads>>>(b, n, m, dataset, weights, temp, idxs); break;
        case 2:
        furthest_point_sampling_weights_kernel<2><<<b, n_threads>>>(b, n, m, dataset, weights, temp, idxs); break;
        case 1:
        furthest_point_sampling_weights_kernel<1><<<b, n_threads>>>(b, n, m, dataset, weights, temp, idxs); break;
        default:
        furthest_point_sampling_weights_kernel<512><<<b, n_threads>>>(b, n, m, dataset, weights, temp, idxs);
    }
